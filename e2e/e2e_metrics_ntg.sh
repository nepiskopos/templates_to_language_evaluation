...
# Usage: ./e2e_metrics_ntg.sh -nar [or -war] -decay ['psql' (or see tabulate docs - only with -mean_stdev)]
...
if [[ $1 != -nar && $1 != -war ]]; then
    echo "Usage: ./e2e_metrics_ntg.sh -decayed 1 [or 0] -nar [or -war] ['psql' (or see tabulate docs - only with -mean_stdev)]"
