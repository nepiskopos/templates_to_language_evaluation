...
# Usage: ./ntg_wb_original.sh -nar [or -war] [-gpu]
...
if [[ $1 != -nar && $1 != -war ]]; then
    echo 'Usage: ./ntg_wb_original.sh -nar [or -war] [-gpu]'
...
if [[ $1 == -nar ]]; then
    ...
    -log_interval 1000 -thresh 29 -emb_drop -bsz 5 -max_seqlen 55 -lr 0.5 -sep_attn -max_pool -unif_lenps -one_rnn -Kmul 3 -mlpinp -onmt_decay $gpu \
    ...
    > /root/neural-template-gen/gens/original_gens/gen-wb-45-3.txt
elif [[ $1 == -war ]]; then
    ...
    > /root/neural-template-gen/gens/original_gens/gen-wb-45-3-war.txt
fi
